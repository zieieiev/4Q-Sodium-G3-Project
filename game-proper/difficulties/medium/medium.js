	...
	for (let i = 0; i < 9; i++) {
		...
		box.addEventListener("click", () => {
			if (i === differentIndex) {
				...
				let highScore = parseInt(localStorage.getItem('highScoreMedium') || '0');
				let highScoreUser = localStorage.getItem('highScoreUserMedium') || '';

				if (score > highScore) {
					highScore = score;
					highScoreUser = currentUsername;
					localStorage.setItem('highScoreMedium', highScore);
					localStorage.setItem('highScoreUserMedium', highScoreUser);
				}
